...
- `serial`: 시퀀스와 기본값을 함께 만드는 편의 문법입니다. 새 설계에서는 명시적인
  `GENERATED ... AS IDENTITY`를 우선 고려합니다.
...
VALUES (
    ...
    ARRAY[23, 19, 15, 18, 22],
    ARRAY['2024-05-17 14:30:00', '2024-05-18 15:00:00', '2024-05-19 16:30:00']
);
...
- `json`과 `jsonb` 두 가지 JSON 데이터 유형을 제공합니다.
...
- 입력 텍스트의 구조와 키 순서를 보존하지만, 처리할 때 파싱 비용이 발생할 수 있습니다.

### JSONB

- 바이너리 형식으로 저장해 조회와 인덱싱에 유리하지만, 입력 시 변환 비용이 있습니다.
- 키 순서와 공백은 보존되지 않으며 중복 키는 마지막 값이 사용됩니다.
