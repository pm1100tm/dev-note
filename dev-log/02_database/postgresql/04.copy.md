...
데이터베이스 복사, `pg_dump`, `pg_restore`를 사용할 때 필요한 명령을 정리합니다.

`CREATE DATABASE ... TEMPLATE ...` 방식은 원본 데이터베이스에 활성 연결이 없을 때만
사용할 수 있습니다. 운영 환경에서는 대상과 원본을 반드시 확인한 뒤 실행합니다.
...
- pg_dump와 pg_restore는 PostgreSQL의 백업과 복원에 사용하는 유틸리티입니다.
- pg_dump는 데이터베이스나 특정 테이블의 내용을 백업하는데 사용
- `pg_restore`는 custom·directory·tar 형식의 덤프를 복원합니다. plain SQL 덤프는
  `psql -f`로 복원합니다.
