...
Java/Spring Boot 프로젝트를 진행하다 보면 아래와 같이 로그를 남기는 코드를 볼 수 있습니다 👇

```java
log.info("Response: " + response);
```

정상적으로 작동하지만, 이런 코드가 누적되면 서비스의 성능을 은근히 잡아먹습니다.

이번 글에서는 SLF4J 로깅의 올바른 사용법과 좋은 예, 나쁜 예를 코드로 비교해봅니다.
...
아래의 한 줄만 있으면, 어떤 로깅 엔진(Logback, Log4j2 등)을 쓰든
`log.info()`, `log.error()`와 같은 공통 문법으로 로그를 남길 수 있습니다.
...
겉보기에는 문제 없어 보이지만, response 가 toString() 연산을 포함한 복잡한 객체일 경우 성능 저하를
유발합니다.

왜냐하면, 로그 레벨이 INFO보다 낮아서 로그가 실제로 출력되지 않더라도 "Response: " + response 문자열
...
| 항목      | 설명                                                            |
| --------- | --------------------------------------------------------------- |
| 🚀 성능   | 로그 레벨이 비활성화(INFO 미사용)되면 문자열 연산 자체가 생략됨 |
