...
- Hibernate 구현체의 스키마 생성 기능이며 JPA 표준 기능이 아닙니다.
- 복잡한 이름 변경·데이터 변환의 의도를 표현하지 못하고, DB와 Hibernate 버전에
  따라 적용 결과가 달라질 수 있습니다.
- 운영 중인 테이블을 예측 없이 변경하면 배포 실패나 데이터 손실 위험이 있습니다. 따라서, 운영 환경에서는
  `update`로 설정하지 않습니다.
...
다른 환경에서 같은 파일이 달라졌는지 검증할 수 있어 팀의 DB 상태를 재현하기 쉬워집니다.
...
- `V`는 versioned migration을 뜻하고, 두 개의 밑줄(`__`)은 버전과 설명의 구분자입니다.
- 각 버전은 고유해야 하며 Flyway는 버전 순서대로 적용합니다.
- 반복 실행할 뷰·프로시저 같은 변경에는 `R__refresh_order_summary.sql`처럼
  repeatable migration을 사용할 수 있습니다.
- repeatable migration은 버전 순서가 아니라 checksum이 바뀌었는지에 따라 다시 적용됩니다.

**한 번 영구 환경에 적용된 versioned migration은 수정하지 않습니다.** 오타나 설계
오류가 발견되면 **_다음 버전 파일을 추가해_** 앞으로 진행합니다. 이미 적용된 파일을
바꾸면 checksum 검증 실패가 나고, 무엇보다 환경마다 실제 DB 변경 내용이 달라질 수 있습니다.
...
- 대용량 backfill, 인덱스 생성, 제약 검증은 온라인 DDL 지원 여부와 락 범위를 DB 공식 문서로 확인합니다.
- Flyway가 SQL을 실행할 수 있다는 사실은 그 SQL이 무중단이라는 보장이 아닙니다.
- DB가 DDL 트랜잭션을 지원하는 범위도 다르므로, 실패 시에는 복구용 migration·백업·배포 중단 기준을 미리 정합니다.
...
- CI에서는 빈 데이터베이스에 `migrate`를 실행하고, 애플리케이션 기동 또는 `validate`로 엔티티 매핑을 확인합니다.
- 변경이 기존 데이터와 함께 동작하는지는 운영과 유사한 데이터 규모의 스테이징 환경에서 별도 검증합니다.
- DB 접속 정보는 환경 변수·비밀 관리 도구로 주입하고 migration 파일이나 CI 로그에 넣지 않습니다.

## flyway history 데이터가 많아질 경우 어떻게 관리하는가

`flyway_schema_history`에 migration 이력이 수백·수천 건 쌓이는 것은 장기 운영
서비스에서 정상입니다. migration당 한 행만 저장하는 메타데이터 테이블이므로,
일반적인 규모에서는 행 수 자체가 성능 문제가 되지 않습니다.

실무에서는 history 행을 삭제하거나 압축하지 않습니다. Flyway는 이력을 기준으로
migration 적용 여부와 checksum을 검증하므로, 행을 직접 수정·삭제하면 이미 적용한
migration을 다시 실행하거나 검증 기준을 잃을 수 있습니다.

- 이미 운영 환경에 적용한 versioned migration 파일과 history 행은 보존합니다.
- `repair`는 행 수를 줄이는 기능이 아닙니다. 실제 DB 상태를 확인한 뒤 실패
  이력이나 checksum 등 메타데이터 문제를 복구할 때만 사용합니다.
- `info`, `validate`, `migrate`가 느릴 때만 실행 시간을 측정하고, history
  테이블 인덱스, DB 연결, migration 파일 탐색 경로 등 실제 병목을 확인합니다.
- 감사·규제 목적의 장기 보관이 필요하면 history 원본은 유지하고, 별도의 감사용
  저장소나 보고 시스템에 복제합니다.

`B...sql` baseline migration은 신규 DB 구축 시간을 줄이는 기능입니다. 기존 운영
DB의 history 행을 줄이거나 대체하지 않습니다.

운영 DB의 history를 비우고 새 기준점부터 시작하는 작업은 일반적인 정리가 아닙니다.
Flyway 관리 체계를 다시 도입하는 수준의 고위험 작업이므로, 특별한 규제 또는 조직 정책상 필요할 때만
DBA·플랫폼 팀과 별도 전환 및 복구 계획을 수립해 진행합니다.
